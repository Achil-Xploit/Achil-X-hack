...
int IntExample2() { if (IsIntExample2) return 1000000000; return old_IntExample2(); }
float get_Balance_Hook() { if (IsBalanceMod) return 1000000000.0f; return (old_Balance != nullptr) ? old_Balance() : 0.0f; }
int GetLevel_Hook() { if (IsMaxLevel) return 1000000000; return old_GetLevel(); }
int GetXP_Hook() { if (IsMaxLevel) return 1000000000; return old_GetXP(); }
...
void SelectNewDriver_Hook(void *instance, int id) { 
    if (instance != NULL && IsUnlockDrivers) { 
        // Kita suntik laci bernama 'driverCount' atau 'maxDrivers' biar jadi 50! 💋
        SetFieldInt(instance, OBFUSCATE("driverCount"), 50); 
    } 
...
inline void (*old_TruckUpdate)(void *instance);
void TruckUpdate_Hook(void *instance) { 
    if (instance != NULL) {
        // 1. Bensin Otomatis Full ⛽💋
        if (IsNoFuelConsum) { 
            SetFieldFloat(instance, OBFUSCATE("fuel"), 1000000000.0f); 
            SetFieldFloat(instance, OBFUSCATE("fuelLeft"), 1000000000.0f); 
        }
