...
void DrawMenu() {
    ...
    if (ImGui::Begin(ICON_FA_GAMEPAD " TheGhost [x64]")) {
        ...
        if (activeTab == 0) {
            ...
            // --- INI DIA SLIDER BOBOT CARGO KAMU SAYANG 📦💋 ---
            ImGui::Separator();
            ImGui::Checkbox("Custom Cargo Weight", &IsLightTrailer);
            if (IsLightTrailer) {
            // Slider buat atur beban (0.1 Kapas - 1.0 Berat Asli)
            ImGui::SliderFloat("Weight", &TrailerWeightValue, 0.1f, 1.0f, "%.1f x");
            }
...
// ==================== [ INSTALASI AUTO CONNECT ] ====================

// 1. Auto Connect Trailer (Biar Nyambung Otomatis 💋)
...
// 2. Auto Connect Fuel & Job (Bensin & Jarak Jadi Satu di Sini ⛽🏁)
Tools::Hook(Il2CppGetMethodOffset(OBFUSCATE("Assembly-CSharp.dll"), OBFUSCATE(""), OBFUSCATE("TruckController"), OBFUSCATE("Update"), 0), (void *)&TruckUpdate_Hook, (void **)&old_TruckUpdate);
